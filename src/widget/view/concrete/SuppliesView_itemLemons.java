...
class SuppliesView_itemLemons extends View {
    ...
    protected void createTextBlocks() {
      ...
      for(int i = 0; i < 3; i++) {
        addTextBlock(new TextBlock(contents[i], new Point(0,0), LTUtil.addPoint(new Point(startingX, startingY + (incrementY * i)), getOffset()), color(255), 14));
        addTextBlock(new TextBlock(contents[3+i], new Point(0,0), LTUtil.addPoint(new Point(startingX, startingY + (incrementY * i) + (CONST.TEXT_INTERLINE/2)), getOffset()), color(255), 14));
      }
    }
    ...
    protected void createTextVariables() { 
      ...
      for(int i = 0; i < 3; i++) {
        addTextVariable(new TextVariable(new Point(0,0), LTUtil.addPoint(new Point(startingX, startingY + (incrementY * i)), getOffset()), color(255), 24));      
      }  
    }
